vec4 GetVolumetricNetherClouds(float depth0, float depth1, vec3 vlAlbedo, float dither, vec4 viewPos) {

	#include "/lib/color/netherColor.glsl"
    vec4 clouds = vec4(0.0);
	vec3 cloudUpColor = netherCol * 2.35;
    vec3 cloudDownColor = cloudUpColor * 0.35;
	
    float cloudAmountM = 0.075 * CLOUD_AMOUNT;

    //Settings
    float cloudAltitude = 60.0;
    float cloudThickness = 50.0;
    int sampleCount = 20;
