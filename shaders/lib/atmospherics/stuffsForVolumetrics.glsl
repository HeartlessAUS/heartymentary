...
float getCloudNoise(vec3 pos) {
	...
	float a = texture2D(noisetex, coord1).x * 0.650;
	float b = texture2D(noisetex, coord2).x * 0.650;
...
float getFogSample(vec3 pos, float height, float verticalThickness, float samples, float amount){
	float ymult = pow(abs(height - pos.y) / verticalThickness, 16);
	...
	noise *= 0.55;
	noise = clamp(noise * 1.00 * amount - (1.0 + ymult), 0.0, 1.0);
	return noise;
}
#endif