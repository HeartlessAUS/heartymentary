...
vec4 getVolumetricFog(float depth0, float depth1, vec3 color, float dither, vec3 nViewPos, float visibility) {
	float maxDist = 512;
    ...
    if (visibility > 0){
		for(int i = 0; i < 12; i++) {
            ...
            if (length(wpos.xz) < maxDist && depth1 > minDist){
                ...
                #else
                float noise = getFogSample(wpos.xyz, 70, 16, 0.4, 0.650);
                #endif
