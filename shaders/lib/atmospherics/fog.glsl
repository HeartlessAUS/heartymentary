...
vec3 Fog1(vec3 color, float lWorldPos, float lViewPos, vec3 nViewPos, vec3 extra, float NdotU) {
		...
		#if FOG1_TYPE < 2
			float fog = lWorldPos / far * 1.5 * (10.0/FOG1_DISTANCE);
		#else
			float fog = lViewPos / far * 1.5 * (10.0/FOG1_DISTANCE);
		#endif
		...
		#if FOG1_TYPE > 0
			float fog = lViewPos / far * (10.0/FOG1_DISTANCE);
		#else
			float fog = lWorldPos / far * (10.0/FOG1_DISTANCE);
		#endif
        fog *= fog;
        fog *= fog;
        fog = 1.0 - exp(-8.0 * fog);
    ...
    #ifdef TWO
		float fog = lWorldPos / far * 4.0 * (10.0/FOG1_DISTANCE);
...
vec3 Fog2(vec3 color, float lViewPos, vec3 worldPos, vec3 extra) {
		...
		float fog2 = lViewPos / pow(far, 0.25) * 0.035 * (1.0 + rainStrengthS) * (32.0/(FOG2_DISTANCE + 0.01));
...
vec3 startFog(vec3 color, vec3 nViewPos, float lViewPos, vec3 worldPos, vec3 extra, float NdotU) {
		...
		if (isEyeInWater == 0) color.rgb = Fog2(color.rgb, lViewPos, worldPos, extra);
	#endif
